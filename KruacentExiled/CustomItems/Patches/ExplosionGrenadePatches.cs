...
namespace KruacentExiled.CustomItems.Patches
{
    public static class ExplosionGrenadePatches
    {
        ...
        public static class ExplodeDestructiblePatch
        {
            public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
            {
                ...
                var argsCtor = Constructor(typeof(OnExplodeDestructibleEventsArgs),
                    new[] { typeof(IDestructible), typeof(float), typeof(ExplosionGrenade) });
                var eventInvoker = Method(typeof(ExplodeEvent), nameof(ExplodeEvent.OnExplodeDestructible));
                var damageGetter = PropertyGetter(typeof(OnExplodeDestructibleEventsArgs), nameof(OnExplodeDestructibleEventsArgs.Damage));

                for (int i = 0; i < codes.Count - 2; i++)
                {
                    // Look for Vector3::op_Addition followed by any stloc.* instruction
                    if (codes[i].Calls(Method(typeof(Vector3), "op_Addition")) &&
                    ...
                    new CodeInstruction(OpCodes.Stloc_2),
                        ...
                        Debug.Log("[ExplosionPatch] Injected OnPreExplodeDestructible successfully");
                        break;
                    }
                }
